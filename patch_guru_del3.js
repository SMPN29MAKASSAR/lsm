...
// Convert to unix endings for easy manipulation
code = code.replace(/\r\n/g, "\n");

const handleHapusStr = `  const handleHapus = async (id: string) => {
...
const newHandleHapusStr = `  const [deleteId, setDeleteId] = useState<string | null>(null);
...
code = code.replace(handleHapusStr, newHandleHapusStr);

const panduanStr = `{showPanduan && <PanduanGuruModal onClose={() => setShowPanduan(false)} />}`;
const newPanduanStr = `{showPanduan && <PanduanGuruModal onClose={() => setShowPanduan(false)} />}
      {deleteId && (
...
code = code.replace(panduanStr, newPanduanStr);
